    ...
    <div class="card">
      <div class="grid gap-4 md:grid-cols-[1fr_220px]">
        <label class="relative block">
          ...
          <input v-model="searchKeyword" class="form-input pl-10" placeholder="搜索申请编号、申请人或产品" />
        </label>
        <select v-model="statusFilter" class="form-input">
          <option value="">全部状态</option>
          <option v-for="status in statusOptions" :key="status" :value="status">{{ getStatusText(status) }}</option>
        </select>
      </div>
    </div>
    ...
    <div class="card overflow-hidden p-0">
      ...
      <div v-if="filteredApplications.length > 0" class="flex items-center justify-between border-t border-gray-100 px-6 py-4 text-sm text-gray-500">
        ...
        <div class="flex items-center gap-2">
          ...
          <span class="px-2 font-medium text-gray-800">{{ currentPage }} / {{ totalPages }}</span>
          <button class="btn-secondary px-3 py-2" :disabled="currentPage === totalPages" @click="currentPage++">
...
import { ChevronLeft, ChevronRight, Eye, FileQuestion, Loader2, RefreshCw, Search } from 'lucide-vue-next'
...
const statusOptions = ['SUBMITTED', 'DOCUMENT_PENDING', 'OCR_PARSING', 'RISK_ANALYZING', 'DECISION_PENDING', 'MANUAL_REVIEW', 'APPROVED', 'REJECTED']

const filteredApplications = computed(() => {
  const keyword = searchKeyword.value.trim().toLowerCase()
  return applications.value.filter((item) => {
    const matchesKeyword = !keyword || [item.applicationNo, item.applicantName, getProductName(item.productCode)]
      .filter(Boolean)
...
const pagedApplications = computed(() => filteredApplications.value.slice((currentPage.value - 1) * pageSize, currentPage.value * pageSize))

const statistics = computed(() => [
  { label: '总申请数', value: applications.value.length, color: 'text-primary-600' },
  { label: '审批通过', value: applications.value.filter((item) => item.status === 'APPROVED').length, color: 'text-success-600' },
  { label: '人工复核', value: applications.value.filter((item) => item.status === 'MANUAL_REVIEW').length, color: 'text-warning-600' },
  { label: '审批拒绝', value: applications.value.filter((item) => item.status === 'REJECTED').length, color: 'text-danger-600' },
])
...
async function loadApplications() {
  ...
  try {
    ...
    applications.value = [...data].sort((a, b) => new Date(b.createdAt || 0) - new Date(a.createdAt || 0))
  } catch (error) {

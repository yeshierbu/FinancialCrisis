<template>
  <div class="animate-fadeIn mx-auto max-w-5xl space-y-6">
    ...
    <div v-else-if="errorMessage" class="card border-red-100 bg-red-50 text-red-700">
      {{ errorMessage }}
    </div>
    ...
    <div v-else-if="!application" class="card text-center">
      ...
      <p class="mt-2 text-sm text-gray-500">请先提交一笔贷款申请，或在申请记录中选择一条记录查看。</p>
      ...
      <section class="card">
        <div class="flex flex-col gap-5 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <h2 class="mt-1 text-2xl font-bold text-gray-950">{{ application.applicationNo }}</h2>
            <p class="mt-2 text-sm text-gray-500">最后更新：{{ formatDate(statusInfo?.lastUpdatedAt || application.updatedAt) }}</p>
          </div>
          <span :class="['status-badge', getStatusClass(statusInfo?.status || application.status)]">
            {{ statusInfo?.statusDesc || getStatusText(application.status) }}
          </span>
        </div>

        <div class="mt-6 grid gap-4 md:grid-cols-4">
          <div v-for="item in summaryItems" :key="item.label" class="rounded-lg bg-gray-50 p-4">
      ...
      <section class="grid gap-6 lg:grid-cols-[1fr_0.85fr]">
          ...
          <div class="space-y-5">
            <div v-for="(event, index) in normalizedTimeline" :key="`${event.status}-${event.time}-${index}`" class="flex gap-4">
              <div class="flex flex-col items-center">
                <span :class="['flex h-9 w-9 items-center justify-center rounded-full', index === normalizedTimeline.length - 1 ? 'bg-primary-600 text-white' : 'bg-success-500 text-white']">
                  <CheckCircle2 class="h-5 w-5" />
                </span>
                <span v-if="index < normalizedTimeline.length - 1" class="mt-2 h-full min-h-8 w-px bg-gray-200"></span>
              </div>
              <div class="pb-2">
                <p class="font-medium text-gray-900">{{ getStatusText(event.status) }}</p>
                <p class="mt-1 text-sm text-gray-500">{{ formatDate(event.time) }}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-6">
          <div class="card">
            ...
            <p class="text-sm leading-6 text-gray-600">{{ application.currentStep || '暂无步骤说明' }}</p>
          </div>

          <div class="card">
            <h3 class="mb-4 text-lg font-semibold text-gray-900">审批报告</h3>
            <template v-if="report">
              <p class="text-sm text-gray-500">报告类型：{{ report.reportType }}</p>
              <p class="mt-2 break-all text-sm text-gray-600">{{ report.reportUrl }}</p>
            </template>
            <p v-else class="text-sm leading-6 text-gray-600">申请审批通过或拒绝后，后端会生成审批报告地址。</p>
          </div>
        </div>
      </section>
    </template>
...
import { CheckCircle2, FileQuestion, Loader2 } from 'lucide-vue-next'

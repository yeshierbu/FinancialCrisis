<template>
  <div class="animate-fadeIn mx-auto max-w-4xl space-y-6">
    ...
    <div class="grid gap-3 md:grid-cols-3">
      <div v-for="(step, index) in steps" :key="step" class="flex items-center gap-3 rounded-lg border border-gray-100 bg-white p-4">
        <span :class="['step-indicator', currentStep >= index ? 'step-active' : 'step-inactive']">
    ...
    <form class="card space-y-6" @submit.prevent="handlePrimaryAction">
      <div v-if="currentStep === 0" class="space-y-6">
        ...
        <div class="grid gap-5 md:grid-cols-2">
          <label class="block">
            ...
            <input v-model.trim="form.applicantName" class="form-input" required placeholder="请输入申请人姓名" />
          </label>
          <label class="block">
            <span class="form-label">身份证号</span>
            <input v-model.trim="form.idCardNo" class="form-input" required placeholder="请输入身份证号" />
          </label>
          <label class="block">
            <span class="form-label">手机号</span>
            <input v-model.trim="form.mobile" class="form-input" required placeholder="请输入手机号" />
          </label>
          <label class="block">
          ...
          <label class="block">
            ...
            <input v-model.number="form.loanAmount" class="form-input" min="1000" required type="number" placeholder="最低 1000 元" />
          </label>
          <label class="block">
      ...
      <div v-else-if="currentStep === 1" class="space-y-6">
        ...
        <div class="grid gap-5 md:grid-cols-2">
          ...
          <label class="block">
            ...
            <input v-model.number="monthlyIncome" class="form-input" min="0" type="number" placeholder="用于前端展示，不提交后端" />
          </label>
        </div>
      </div>
      ...
      <div v-else class="space-y-6">
        <div class="flex items-start justify-between gap-4">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-500">当前后端接收材料元数据，不上传真实文件内容。</p>
          </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
          <div v-for="doc in documents" :key="doc.type" class="rounded-lg border border-dashed border-gray-200 p-5">
            <div class="mb-4 flex h-11 w-11 items-center justify-center rounded-lg bg-gray-100 text-gray-500">
            ...
            <p v-if="doc.file" class="mt-3 truncate text-sm font-medium text-success-600">{{ doc.file.name }}</p>
        ...
        <label class="flex items-start gap-3 rounded-lg bg-gray-50 p-4 text-sm text-gray-600">
          <input v-model="agreed" class="mt-1" type="checkbox" />
          <span>我确认提交的信息真实有效，并同意用于本次智能信贷审批演示。</span>
        </label>
      </div>

      <div v-if="errorMessage" class="rounded-lg border border-red-100 bg-red-50 p-4 text-sm text-red-700">
        {{ errorMessage }}
      </div>
      ...
      <div class="flex justify-between border-t border-gray-100 pt-6">
        ...
        <button type="submit" class="btn-primary" :disabled="submitting || !canContinue">
...
import { CheckCircle, FileText, IdCard, Loader2, Upload, WalletCards } from 'lucide-vue-next'
...
const canContinue = computed(() => {
  if (currentStep.value === 0) {
    return form.applicantName && form.idCardNo && form.mobile && form.productCode && form.loanAmount >= 1000 && form.loanTerm > 0
  }
